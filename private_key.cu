...
__global__ void profanity_init(const point * const precomp, mp_number * pDeltaX, mp_number * pPrevLambda, const cl_ulong4 seed, const cl_ulong4 seedX, const cl_ulong4 seedY) {
...
__global__ void profanity_inverse(const mp_number * pDeltaX, mp_number * pInverse) {
    	const size_t id = (threadIdx.x + blockIdx.x * blockDim.x);
...
__global__ void profanity_iterate(mp_number * pDeltaX, mp_number * pInverse, mp_number * pPrevLambda) {
    	const size_t id = (threadIdx.x + blockIdx.x * blockDim.x);
...
//__global__ void profanity_init(const point * const precomp, mp_number& pDeltaX, mp_number& pPrevLambda, result * const pResult, const ulong4 seed, const ulong4 seedX, const ulong4 seedY) {
